...
export class CertificatesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.firebaseService.getCertificates()
      .then((values) => {
  ...
  handleClick(certificateDetails: {rowData: CertificateElement, event: Event}) {
    if (!this.selectedRow || certificateDetails['rowData'].index !== this.selectedRow.index) {
